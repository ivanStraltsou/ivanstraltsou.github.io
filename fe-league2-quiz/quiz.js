(function() {
  ...
  function showResults() {
    ...
    if (quiz.checkAnswers()) {
      var quizScorePercent = quiz.result.scorePercentFormatted; // The unformatted percentage is a decimal in range 0 -
                                                                // 1
      ...
      // Highlight questions according to whether they were correctly answered. The callback allows us to
      // highlight/show the correct answer
    ...
    if (!correct) {
      ...
      for (var i = 0; i < answers.length; i++) {
        if (answers[i].type === "checkbox" || answers[i].type === "radio") {
          ...
          correctAnswer.classList.add(quiz.Classes.TEMP); // quiz.checkAnswers will automatically remove elements with
                                                          // the temp class
